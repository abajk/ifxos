...
/* ============================================================================
   IFX VxWorks adaptation - Task handling
   ========================================================================= */
...
/**
   VxWorks - Thread stub function. The stub function will be called
...
/**
   VxWorks - Creates a new task.

\par Implementation
   - Allocate and setup the internal thread control structure.
   - setup the VxWorks specific thread parameter.
   - start the VxWorks task with the internal stub function (see "taskSpawn")
               ...
               IFX_ulong_t    nArg2)
{
   ...
   if(pThrCntrl)
   {
      if (IFXOS_THREAD_INIT_VALID(pThrCntrl) == IFX_FALSE)
      {
         ...
         if((pThrCntrl->tid = taskSpawn (
                        pThrCntrl->thrParams.pName,
                        nPriority,
                        IFXOS_DRV_THREAD_OPTIONS,
                        nStackSize,
                        IFXOS_ThreadStartup,
                        (int)pThrCntrl, 0, 0, 0, 0, 0, 0, 0, 0, 0)) == ERROR)
...
/**
   VxWorks - Shutdown and terminate a given thread.
         ...
         if (pThrCntrl->thrParams.bRunning == 1)
         {
            IFXOS_PRN_USR_ERR_NL( IFXOS, IFXOS_PRN_LEVEL_WRN,
               ("IFXOS WRN - Thread Delete <%s> TID %d - kill, no shutdown "
                "response" IFXOS_CRLF, pThrCntrl->thrParams.pName,
                pThrCntrl->tid));
...
/**
   VxWorks - Shutdown a given thread.
   Therefore the thread delete functions triggers the user thread function
...
/**
   VxWorks - Modify own thread priority.
