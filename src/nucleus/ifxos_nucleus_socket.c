...
/** \addtogroup IFXOS_SOCKET_NUCLEUS
@{ */

#if ( defined(IFXOS_HAVE_SOCKET) && (IFXOS_HAVE_SOCKET == 1) )

/**
   Nucleus - This function init and setup the socket feature on the system.

\par Implementation
   - Nothing under Nucleus.
...
/**
   Nucleus - This function cleanup the socket feature on the system.
...
/**
   Nucleus - This function creates a TCP/IP, UDP/IP or raw socket.
...
/**
   Nucleus - Pend (wait) on multiple file descriptors.

\par Implementation
   -  via "select"

\param
   socFd          specifies the file descriptor number of the socket
\param
...
\param
   nTimeout       specifies behavior if event is not available:
...
/**
   Nucleus - Receives data from a connected socket.
...
/**
   Nucleus - Receives data from a datagram socket.
...
/**
   Nucleus - Sends data to connected socket.
...
/**
   Nucleus Application - Sends data to UDP socket.
...
/**
   Nucleus - Indicates that the server is willing to accept connection requests from
...
/**
   Nucleus - Accept a Connection from the socket.
...
/**
   Nucleus - Establish a connection by a TCP client.
...
/**
   Nucleus - This function converts a network address to dotted decimal notation.
...
/**
   Nucleus - Mark a descriptor in use.
...
/**
   Nucleus - Clear a given descriptor.
...
/**
   Nucleus - Check if a descriptor is set.
...
/**
   Nucleus - Clear a descriptor mask.
